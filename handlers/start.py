...
async def start(msg, user, language):
    ...
    logger.debug(f"[<y>{msg.from_user.username}</y>]: User registration has started")
...
@dp.message_handler(IsPrivate, ContextButton("cancel", LANGUAGES), state=Register.all_states)
async def cancel(msg, state, language):
    await state.finish()

    logger.debug(f"[<y>{msg.from_user.username}</y>]: Cancellation of registration")
...
async def enter_name(msg, state, language):
    logger.debug(f"[<y>{msg.from_user.username}</y>]: The user entered a name \"{msg.text}\"")
...
async def enter_age(msg, state, language):
    if not msg.text.isnumeric() or not (8 < int(msg.text) < 100):
        logger.debug(f"[<y>{msg.from_user.username}</y>]: The user made a mistake when entering the age \"{msg.text}\"")
        return await msg.reply(context[language].invalid_input)

    async with state.proxy() as data:
        data['age'] = int(msg.text)

    logger.debug(f"[<y>{msg.from_user.username}</y>]: The user entered the age \"{msg.text}\"")
    await Register.next()
...
async def enter_phone_number(msg, state, language):
    if not msg.text.startswith("+") or not msg.text.strip("+").isnumeric():
        logger.info(f"[<y>{msg.from_user.username}</y>]: The user made a mistake when entering the phone number \"{msg.text}\"")
        return await msg.reply(context[language].invalid_input)
    ...
    logger.debug(f"[<y>{msg.from_user.username}</y>]: The user entered a phone number \"{msg.text}\"")
    ...
    try:
        db.session.commit()
        logger.info(f"[<y>{msg.from_user.username}</y>]: The user has registered")
        return await msg.reply(context[language].user_registered, reply_markup=ReplyKeyboardRemove())
    except Exception:
        db.session.rollback()
        db.session.remove()
        logger.exception(f"[<y>{msg.from_user.username}</y>]: Registration failed")
